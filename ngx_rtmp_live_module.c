...
extern ngx_cache_global_list_t *g_cache_list;
...
ngx_rtmp_live_get_stream(ngx_rtmp_session_t *s, u_char *name, int create, int pubflag)
{
    ngx_rtmp_live_app_conf_t   *lacf;
	...
	if(g_cache_list && create && pubflag){
		...
		list_for_each_safe(pos, npos, &(g_cache_list->list)){
			...
			if(!strcmp((char*)name, node->playpath)){
				if(node->active && (ngx_rtmp_session_t*)(node->session) != s){
					pthread_mutex_unlock(&(g_cache_list->lock));
					find = 1;
					break;
				}
				node->newsession = (void*)s;
				node->active = 1;
				pthread_mutex_unlock(&(g_cache_list->lock));
				find = 1;
				
				if(node->pid){
					pthread_join(node->pid, NULL);
					node->pid = 0;
				}			

				if(node){
					ngx_rtmp_relay_close(node->session);
					ngx_rtmp_free_session(node->session);
					node->session = (void*)s;
				}
				break;
			}
...
ngx_rtmp_live_join(ngx_rtmp_session_t *s, u_char *name, unsigned publisher)
{
    ...
    stream = ngx_rtmp_live_get_stream(s, name, publisher || lacf->idle_streams, publisher);
...
ngx_rtmp_live_close_stream(ngx_rtmp_session_t *s, ngx_rtmp_close_stream_t *v)
{
    ...
    stream = ngx_rtmp_live_get_stream(s, ctx->stream->name, 0, 0);
    if (stream == NULL) {
        goto next;
    }
    *stream = (*stream)->next;
