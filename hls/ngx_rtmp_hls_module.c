...
ngx_cache_global_list_t *g_cache_list = NULL;
...
static ngx_int_t
ngx_rtmp_hls_video1(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h, ngx_chain_t *in, int first);

static ngx_int_t
ngx_rtmp_hls_audio1(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h,
    ngx_chain_t *in);

static void
ngx_rtmp_hls_update_fragment1(ngx_rtmp_session_t *s, uint64_t ts, uint64_t next,
    ngx_int_t boundary, ngx_uint_t flush_rate);

static void *
ngx_join_over_pthread(void *data){
...
static void
ngx_finish_pthread(pthread_t pid){
	...
	pthread_create(&ntid, &attr, ngx_join_over_pthread, (void*)&pid);

	pthread_attr_destroy(&attr);
}

static ngx_int_t
ngx_rtmp_hls_copy_frag(ngx_rtmp_session_t *s1, ngx_rtmp_session_t *s2)
{
	ngx_rtmp_hls_ctx_t             *ctx1, *ctx2;
    ngx_rtmp_hls_frag_t            *f1, *f2;
	ngx_uint_t i;
	if(!s1 || !s2){
		ngx_log_error(NGX_LOG_INFO, s2->connection->log, 0, "===1==\n");
		return NGX_OK;
	}
    ctx1 = ngx_rtmp_get_module_ctx(s1, ngx_rtmp_hls_module);
	ctx2 = ngx_rtmp_get_module_ctx(s2, ngx_rtmp_hls_module);

	if(!ctx1 || !ctx2){
		ngx_log_error(NGX_LOG_INFO, s2->connection->log, 0, "===2==\n");
		return NGX_OK;
	}
	ctx1->frag = ctx2->frag;
	ctx1->nfrags = ctx2->nfrags;
	ctx1->frag_ts = ctx2->frag_ts;
	ctx1->opened = ctx2->opened;

	ctx1->key_id = ctx2->key_id;
	ctx1->audio_cc = ctx2->audio_cc;
	ctx1->video_cc = ctx2->video_cc;
	ctx1->key_frags = ctx2->key_frags;
	ctx1->aframe_base = ctx2->aframe_base;
	ctx1->aframe_num = ctx2->aframe_num;
	ctx1->aframe_pts = ctx2->aframe_pts;	

	ngx_memcpy(&ctx1->file, &ctx2->file, sizeof(ngx_rtmp_mpegts_file_t));
	ctx1->file.log = NULL;

    for (i = 0; i <= ctx2->nfrags; i++) {
        f1 = ngx_rtmp_hls_get_frag(s1, i);
		f2 = ngx_rtmp_hls_get_frag(s2, i);
		if(!f2){
			ngx_log_error(NGX_LOG_INFO, s2->connection->log, 0, "===3==\n");
			continue;
		}
        ngx_memcpy(f1, f2, sizeof(ngx_rtmp_hls_frag_t));
    }
	return NGX_OK;
}

static void *
ngx_close_stream_flush_buf(void *data){

	struct list_head 				*ppos=NULL, *pnops=NULL, *cpos=NULL, *cnpos=NULL;
	ngx_split_list_t 				*plnode=NULL, *cur_plnode=NULL;
	ngx_cache_list_t 				*clnode=NULL;
	ngx_rtmp_hls_ctx_t				*ctx;
	ngx_rtmp_session_t 				*s = NULL;
	char 							strpath[1024]={0};
	int								need=0;
	struct timeval 					tv;

	s = (ngx_rtmp_session_t*)data;
	ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_hls_module);
	if (ctx == NULL){
		ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "get ctx error");
		return NULL;
	}
	ngx_memcpy(strpath, ctx->name.data, ctx->name.len < 1023 ? ctx->name.len : 1023);

	if(g_cache_list){
		pthread_mutex_lock(&(g_cache_list->lock));
		list_for_each_safe(ppos, pnops, &(g_cache_list->list)){
			plnode = list_entry(ppos, ngx_split_list_t, node);
			if(!strcmp(strpath, plnode->playpath)){
				cur_plnode = plnode;
				break;
			}
		}
		pthread_mutex_unlock(&(g_cache_list->lock));
	}
	if(!cur_plnode)
		return NULL;

	list_for_each_safe(cpos, cnpos, &(cur_plnode->list)){
		clnode = list_entry(cpos, ngx_cache_list_t, node);

		if(cur_plnode->clear){
			goto cacheclear;
		}
		if(cur_plnode->active){
			goto out;
		}

		ngx_rtmp_header_t *oh;
		ngx_chain_t *ion;
		struct list_head *pos, *npos;
		ngx_cache_node_t *node;
		list_for_each_safe(pos, npos, &(clnode->list)){
			node = list_entry(pos, ngx_cache_node_t, node);
			...
			for(;;){
				...
				ngx_gettimeofday(&tv);
				if((int)(tv.tv_sec - node->tv.tv_sec) < NGX_CACHE_IN_TIME_LEVEL){
					ngx_msleep(10);
					continue;
				} 
				if((int)(tv.tv_sec - node->tv.tv_sec) == NGX_CACHE_IN_TIME_LEVEL){
					if(tv.tv_usec < node->tv.tv_usec){
						ngx_msleep(10);
						continue;
					}
				}
			...
			if(cur_plnode->open){
				...
				if(cur_plnode->lcount > 1)
					cur_plnode->lcount--;

				ngx_rtmp_hls_flush_audio(s);
				ngx_rtmp_hls_update_fragment1(s, cur_plnode->dts*90, oh->timestamp*90, 1, 1);
			}
			cur_plnode->dts = oh->timestamp;
			
			if(oh->type == NGX_RTMP_MSG_AUDIO){
				ngx_rtmp_hls_audio1(s, oh, ion);
			} else {
				if(cur_plnode->newseg){
					ngx_rtmp_hls_video1(s, oh, ion, cur_plnode->newseg);
					cur_plnode->newseg = 0;
				} else
					ngx_rtmp_hls_video1(s, oh, ion, cur_plnode->newseg);
			}

			ngx_cache_node_free(node);
		}

		if(list_empty(&(clnode->list))){
			list_del(cpos);
			ngx_cache_list_free(clnode);
			cur_plnode->open = 1;
		}
ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, 
			"a cache list was writed out");
	}

cacheclear:

	pthread_mutex_lock(&(g_cache_list->lock));
	if(cur_plnode->clear || list_empty(&(cur_plnode->list))){
		if(cur_plnode->active){
			pthread_mutex_unlock(&(g_cache_list->lock));
			cur_plnode->lcount = 0;
			goto out;
		...
		cur_plnode->finalclose = 1;
		ngx_rtmp_relay_close(s);
		ngx_rtmp_free_session(s);

if(cur_plnode->clear)
			ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
		                        "final clear buffer: name='%s'", strpath);

		ngx_split_list_free(cur_plnode);
	}
	pthread_mutex_unlock(&(g_cache_list->lock));

	ngx_finish_pthread(pthread_self());
...
out:
	if(need){
	ngx_rtmp_hls_flush_audio(s);
	ngx_rtmp_hls_copy_frag((ngx_rtmp_session_t*)cur_plnode->newsession, s);
	}
...
{
	struct list_head *pos, *npos;
	ngx_split_list_t *node=NULL;
	
	if(g_cache_list){
		pthread_mutex_lock(&(g_cache_list->lock));
		list_for_each_safe(pos, npos, &(g_cache_list->list)){
			node = list_entry(pos, ngx_split_list_t, node);
			if(!ngx_strcmp(path, node->playpath)){
				node->clear = 1;
				break;
			}
		}
...
ngx_rtmp_check_stream_active(ngx_rtmp_session_t *s)
{
	...
	ngx_rtmp_hls_ctx_t             *ctx;
	ngx_rtmp_hls_app_conf_t        *hacf;
	struct list_head 				*pos, *npos; //*cpos, *cnpos;
	char 							strpath[1024]={0};
	ngx_split_list_t 				*node=NULL;
	...
	if(g_cache_list){
		...
		list_for_each_safe(pos, npos, &(g_cache_list->list)){
			node = list_entry(pos, ngx_split_list_t, node);
			if(!strcmp((char*)strpath, node->playpath)){
				if((ngx_rtmp_session_t*)node->session != s){
					ret = 2;
					break;
				}
				ret = node->active;
...
static ngx_int_t
ngx_rtmp_hls_close_stream1(ngx_rtmp_session_t *s, ngx_rtmp_close_stream_t *v)
{
    ngx_rtmp_hls_app_conf_t        *hacf;
    ngx_rtmp_hls_ctx_t             *ctx;
...
ngx_rtmp_hls_close_stream(ngx_rtmp_session_t *s, ngx_rtmp_close_stream_t *v)
{
	...
	if(g_cache_list){
		...
		list_for_each_safe(pos, npos, &(g_cache_list->list)){
			node = list_entry(pos, ngx_split_list_t, node);
			if(!strcmp(strpath, node->playpath)){
				if((ngx_rtmp_session_t*)(node->session) != s){
					pthread_mutex_unlock(&(g_cache_list->lock));
					return ngx_rtmp_hls_close_stream1(s, v);
				}
				
				if(node->active && !node->finalclose){
					node->active = 0;
					node->newpublish = 1;

					err = pthread_create(&node->pid, NULL, ngx_close_stream_flush_buf, (void*)s);
					if(err != 0){
						pthread_mutex_unlock(&(g_cache_list->lock));
						ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "pthread_create() failed");
						return NGX_ERROR;
					}
				} else {
					node->finalclose = 0;
    ...
    ngx_int_t boundary, ngx_uint_t flush_rate)
{
    ...
    if (boundary) {
        ngx_rtmp_hls_close_fragment(s);
        ngx_rtmp_hls_open_fragment(s, next, discont);
    }
...
ngx_rtmp_hls_av(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h,
    ngx_chain_t *in)
{
	ngx_rtmp_header_t				*ih=NULL, *oh=NULL;
	ngx_cache_node_t				*inode=NULL, *node=NULL;
	ngx_chain_t 					*iin=NULL, *ion=NULL;
	ngx_buf_t						*b=NULL;
	u_char							*p=NULL;
	ngx_cache_list_t				*li = NULL;
	struct list_head 				*pos=NULL, *npos=NULL;
	ngx_rtmp_hls_ctx_t              *ctx=NULL;
	char 							strpath[1024]={0};
	ngx_split_list_t 				*plnode=NULL, *cur_plnode=NULL;
	struct timeval 					tv;
	...
	inode = ngx_cache_node_create();
	if(!inode){
		ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "malloc 1 failed");
		return NGX_ERROR;
	}
	inode->h = ih= malloc(sizeof(ngx_rtmp_header_t));
	if(!ih){
		ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "malloc 2 failed");
		return NGX_ERROR;
	}
	memset(ih, 0, sizeof(ngx_rtmp_header_t));
	memcpy(ih, h, sizeof(ngx_rtmp_header_t));
	
	inode->in = iin = malloc(sizeof(ngx_chain_t));
	if(!ih){
		ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "malloc 3 failed");
		return NGX_ERROR;
	}
	memset(iin, 0, sizeof(ngx_chain_t));
	iin->buf = b = malloc(sizeof(ngx_buf_t));
	if(!b){
		ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "malloc 4 failed");
		return NGX_ERROR;
	...
	b->start = malloc(h->mlen);
	if(!b->start){
		ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "malloc 5 failed");
		return NGX_ERROR;
	}
	memset(b->start, 0, h->mlen);
	b->end = b->start + h->mlen;
	b->pos = b->start;
	b->last = b->pos;
	iin->next = NULL;

	p = in->buf->pos;
	if (ngx_rtmp_hls_copy(s, b->last, &p, h->mlen, &in) != NGX_OK) {
		return NGX_ERROR;
	}
	b->last += h->mlen;

	if(!g_cache_list){
		g_cache_list = ngx_cache_global_list_create();
	}
	...
	if(1){
		...
		pthread_mutex_lock(&(g_cache_list->lock));
		list_for_each_safe(pos, npos, &(g_cache_list->list)){
			plnode = list_entry(pos, ngx_split_list_t, node);
			if(!strcmp(strpath, plnode->playpath)){
				if(plnode->clear == 1){
					ngx_rtmp_hls_flush_audio(s);
					ngx_rtmp_hls_close_fragment(s);
					...
					list_del(pos);
					ngx_split_list_free(plnode);
					
					plnode = ngx_split_list_create();
					if(!plnode){
						ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
				                      "malloc 6 failed");
						return NGX_ERROR;
					}
					sprintf(plnode->playpath, "%s", strpath);
					plnode->session = (void*)s;
					list_add_tail(&(plnode->node), &(g_cache_list->list));
					
					ngx_cache_list_t *clist = ngx_cache_list_create();
					if(!clist){
						ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
				                      "malloc 7 failed");
						return NGX_ERROR;
					}
					list_add_tail(&(clist->node), &(plnode->list));
					plnode->lcount++;
					plnode->curlist = &(clist->node);
					plnode->open = 1;
					
					ctx->frag_ts = (uint64_t) h->timestamp * 90;

					ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
		                        	"clear buffer: name='%s'", strpath);
				}
				cur_plnode = plnode;
				break;
			}
		...
		if(!cur_plnode){
			ngx_split_list_t *plist = ngx_split_list_create();
			if(!plist){
				ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
		                      "malloc 8 failed");
				return NGX_ERROR;
			}
			sprintf(plist->playpath, "%s", strpath);
			plist->session = (void*)s;
			list_add_tail(&(plist->node), &(g_cache_list->list));
			...
			list_add_tail(&(clist->node), &(cur_plnode->list));
			cur_plnode->lcount++;

			li = clist;
			cur_plnode->curlist = &(clist->node);
		} else {
			
			if(cur_plnode->newpublish){
				cur_plnode->newpublish = 0;
				...
				li = list_entry(cur_plnode->curlist, ngx_cache_list_t, node);
			}
		}
	...
	if(1){
		...
		ngx_gettimeofday(&(inode->tv));
		ngx_memcpy(&tv, &(inode->tv), sizeof(struct timeval));
		list_add_tail(&(inode->node), &(li->list));
	}


	///
	ngx_cache_list_t *cli;
	list_for_each_safe(pos, npos, &(cur_plnode->list)){
		cli = list_entry(pos, ngx_cache_list_t, node);
		
		struct list_head *lpos, *lnpos;
		list_for_each_safe(lpos, lnpos, &(cli->list)){
			node = list_entry(lpos, ngx_cache_node_t, node);
			
			if(!cli->full && (int)(tv.tv_sec - node->tv.tv_sec) < NGX_CACHE_IN_TIME_LEVEL){
				return NGX_OK;
			}
			if(!cli->full && (int)(tv.tv_sec - node->tv.tv_sec) == NGX_CACHE_IN_TIME_LEVEL){
				if(tv.tv_usec < node->tv.tv_usec){
...
static ngx_int_t
ngx_rtmp_hls_video1(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h,
    ngx_chain_t *in, int first)
{
    ngx_rtmp_hls_app_conf_t        *hacf;
    ngx_rtmp_hls_ctx_t             *ctx;
    ngx_rtmp_codec_ctx_t           *codec_ctx;
