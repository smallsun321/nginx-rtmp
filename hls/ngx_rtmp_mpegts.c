...
ngx_cache_node_t* 
ngx_cache_node_create()
{
	ngx_cache_node_t *node = NULL;
	node = (ngx_cache_node_t*)malloc(sizeof(ngx_cache_node_t));
	if(node == NULL)
		return NULL;
	memset(node, 0, sizeof(ngx_cache_node_t));
...
ngx_cache_list_t* 
ngx_cache_list_create()
{
	ngx_cache_list_t *li = NULL;
	li = (ngx_cache_list_t*)malloc(sizeof(ngx_cache_list_t));
	if(li == NULL)
		return NULL;
	memset(li, 0, sizeof(ngx_cache_list_t));

	INIT_LIST_HEAD(&(li->node));
...
ngx_int_t 
ngx_cache_node_free(ngx_cache_node_t *node)
{
	if(node){
		if(node->h){
			free((void*)(node->h));
		...
		if(node->in){
			if(node->in->buf){
				if(node->in->buf->start){
					free((void*)(node->in->buf->start));
					node->in->buf->start = NULL;
				}
				free((void*)(node->in->buf));
				node->in->buf = NULL;
				node->in->next = NULL;
			}			
			free((void*)(node->in));
...
ngx_int_t
ngx_cache_list_free(ngx_cache_list_t *list)
{
...
ngx_cache_global_list_create()
{
	ngx_cache_global_list_t *li = NULL;
	li = (ngx_cache_global_list_t*)malloc(sizeof(ngx_cache_global_list_t));
	if(li == NULL)
		return NULL;
	memset(li, 0, sizeof(ngx_cache_global_list_t));
...
ngx_int_t 
ngx_cache_global_list_free(ngx_cache_global_list_t *list)
{
	struct list_head *pos, *npos;
	ngx_split_list_t *node;
...
ngx_split_list_t*	
ngx_split_list_create(){
	ngx_split_list_t *li = NULL;
	li = (ngx_split_list_t*)malloc(sizeof(ngx_split_list_t));
	if(li == NULL)
		return NULL;
	memset(li, 0, sizeof(ngx_split_list_t));

	INIT_LIST_HEAD(&(li->node));
	INIT_LIST_HEAD(&(li->list));

	memset(li->playpath, 0, 1024);
...
ngx_int_t 
ngx_split_list_free(ngx_split_list_t *list)
{
	struct list_head *pos, *npos;
	ngx_cache_list_t *node;

	if(list){
		list->curlist = NULL;
		list->session = NULL;
		list->newsession = NULL;

		list_for_each_safe(pos, npos, &(list->list)){
			node = list_entry(pos, ngx_cache_list_t, node);
			list_del(pos);
			ngx_cache_list_free(node);
		}
