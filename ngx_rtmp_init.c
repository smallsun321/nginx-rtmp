...
static void
ngx_rtmp_close_session_handler1(ngx_event_t *e)
{
    ngx_rtmp_session_t                 *s;
    ngx_connection_t                   *c;
    ngx_rtmp_core_srv_conf_t           *cscf;
...
ngx_rtmp_close_session_handler(ngx_event_t *e)
{
	...
	ngx_rtmp_session_t                 *s;
	s = e->data;
	ret = ngx_rtmp_check_stream_active(s);
	if(ret == 1){
		ngx_rtmp_cmd_disconnect_init(s, NULL, NULL);
		return;
	}
	if(ret == 0){
		return;
	...
	if(ret == 2){
		return ngx_rtmp_close_session_handler1(e);
	}
}

void
ngx_rtmp_free_session(ngx_rtmp_session_t *s)
{
    ngx_connection_t                   *c;
    ngx_rtmp_core_srv_conf_t           *cscf;
    c = s->connection;
